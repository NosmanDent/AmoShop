...
import { getProductDataSkin } from "../ProductStoreSkin";
import { Link } from "react-router-dom";
import { GrClose } from "react-icons/gr";
import { BsStarFill, BsStar, BsStarHalf } from "react-icons/bs";

const ProductDetailsSkin = () => {
  const { id } = useParams(); // Get the ID from the URL parameter
  const product = getProductDataSkin(id); // Get the product data based on the ID

  const cartContext = useContext(CartContext);
  const { getProductQuantity, addToCart, removeFromCart, deleteFromCart } =
    cartContext;
  const productQuantity = getProductQuantity("skin", product.id);
  ...
  return (
    <div className=" px-6 md:px-8 mt-5 flex flex-col h-screen w-full mb-[600px] md:mb-[200px]">
      <Link to="/shop/skincare" className="mb-10">
        <GrClose />
      </Link>
              ...
              <>
                <div className="flex  flex-row md:flex-col gap-2 item-center justify-center  w-full ">
                  ...
                  <button
                    ...
                    onClick={() => addToCart("skin", product.id)}
                  >
                    +
                  </button>
                  <button
                    className="rounded-full bg-[#68BB93] text-white px-4 py-2 mx-1"
                    onClick={() => removeFromCart("skin", product.id)}
                  >
                    -
                  </button>
                  <button
                    className=" text-sm md:text-xl m-0 md:m-auto"
                    onClick={() => deleteFromCart("skin", product.id)}
                  >
                    <FcEmptyTrash />
                  </button>
                </div>
...
export default ProductDetailsSkin;
