...
import { getProductDataKids } from "../ProductStoreKids";
...
const ProductDetailsKids = () => {
  ...
  const product = getProductDataKids(id); // Get the product data based on the ID

  const cartContext = useContext(CartContext);
  const { getProductQuantity, addToCart, removeFromCart, deleteFromCart } =
    cartContext;
  const productQuantity = getProductQuantity("kids", product.id);
  ...
  return (
    <div className=" px-6 md:px-8 mt-5 flex flex-col h-screen w-full mb-[600px] md:mb-[200px]">
      <Link to="/shop/kidsandbabies" className="mb-10">
        <GrClose />
      </Link>
              ...
              <>
                <div className="flex  flex-row md:flex-col gap-2 item-center justify-center  w-full ">
                  ...
                  <button
                    ...
                    onClick={() => addToCart("kids", product.id)}
                  >
                    +
                  </button>
                  <button
                    className="rounded-full bg-[#68BB93] text-white px-4 py-2 mx-1"
                    onClick={() => removeFromCart("kids", product.id)}
                  >
                    -
                  </button>
                  <button
                    className=" text-sm md:text-xl m-0 md:m-auto"
                    onClick={() => deleteFromCart("kids", product.id)}
                  >
                    <FcEmptyTrash />
                  </button>
                </div>
